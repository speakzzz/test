...
if( st[1][0] != '#' ) // Didn't find a hash?
{
	...
	if (option == "SHOWVERIFY")
	{
		if (value == "ON")
		{
			theUser->setFlag(sqlUser::F_SHOWVERIFY);
			theUser->commit(theClient);
			bot->Notice(theClient,"SHOWVERIFY is now ON for you.");
			return true;
		}

		if (value == "OFF")
		{
			theUser->removeFlag(sqlUser::F_SHOWVERIFY);
			theUser->commit(theClient);
			bot->Notice(theClient,"SHOWVERIFY is now OFF for you.");
			return true;
		}
   ...
   if ((option == "NICK") || (option == "NICKNAME"))
   {
	...
	{
		int admLevel = bot->getAdminAccessLevel(theUser);
		if (!admLevel || admLevel < level::admin::potus)
		{
			/* not an admin, return unknown command */
			bot->Notice(theClient,
				bot->getResponse(theUser,
		...
		if (st.size() < 4)
		{
			bot->Notice(theClient,"SYNTAX: SET COORDINATOR <user> <ON|OFF>");
			return false;
		}
		else
			value = string_upper(st[3]);	
		...
		if (value == "ON")
		{
			targetUser->setFlag(sqlUser::F_COORDINATOR);
			targetUser->commit(theClient);
			bot->Notice(theClient,"Set COORDINATOR to ON for user %s", targetUser->getUserName().c_str());

			return true;
		}
		if (value == "OFF")
		{
			targetUser->removeFlag(sqlUser::F_COORDINATOR);
			targetUser->commit(theClient);
			bot->Notice(theClient,"Set COORDINATOR to OFF for user %s", targetUser->getUserName().c_str());
